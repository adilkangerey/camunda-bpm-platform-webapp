...
const loadConfig = (async function() {
  ...
  window.camCatalogConf = config;
  return config;
})();

window.__define(
  'camunda-catalog-bootstrap',
  ['./scripts/camunda-catalog-ui'],
  function() {
    'use strict';
    const bootstrap = function(config) {
      var camundaCatalogUi = window.CamundaCatalogUi;
      ...
      camundaCatalogUi.exposePackages(requirePackages);
      ...
      requirejs(['globalize'], function(globalize) {
        ...
        pluginPackages = pluginPackages.filter(
          el =>
            el.name === 'catalog-plugin-catalogPlugins' ||
            el.name === 'catalog-plugin-catalogEE' ||
            el.name.startsWith('catalog-plugin-legacy')
        );

        pluginDependencies = pluginDependencies.filter(
          el =>
            el.requirePackageName === 'catalog-plugin-catalogPlugins' ||
            el.requirePackageName === 'catalog-plugin-catalogEE' ||
            el.requirePackageName.startsWith('catalog-plugin-legacy')
        );
        ...
        requirejs(dependencies, function(angular) {
          ...
          if (typeof config !== 'undefined' && config.requireJsConfig) {
            ...
            requirejs(custom.deps || [], function() {
              ...
              angular.module('cam.catalog.custom', custom.ngDeps);
              ...
              // initialize Admin
              camundaCatalogUi(pluginDependencies);
            });
          } else {
            // for consistency, also create a empty module
            angular.module('cam.catalog.custom', []);
...
requirejs(['camunda-catalog-bootstrap'], function() {});
