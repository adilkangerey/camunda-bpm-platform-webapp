...
module.exports = function(pluginDependencies) {
  ...
  var ModuleConfig = [
    ...
    function(
      ...
      UriProvider.replace(':appName', 'catalog');
      ...
      UriProvider.replace(
        'admin://',
        getUri('admin-api') || getUri('catalog-api') + '../admin/'
      );
      UriProvider.replace('plugin://', getUri('cockpit-api') + 'plugin/');
      UriProvider.replace('engine://', getUri('engine-api'));
      UriProvider.replace('catalog://', getUri('catalog-api'));
      UriProvider.replace('plugin://', getUri('catalog-api') + 'plugin/');
      UriProvider.replace('engine://', getUri('engine-api'));
      ...
      UriProvider.replace(':engine', [
        ...
        function($window) {
          ...
          var match = uri.match(/\/app\/catalog\/([\w-]+)(|\/)/);
          if (match) {
            return match[1];
          } else {
  ...
  appNgModule.controller('camCatalogAppCtrl', [
    '$scope',
    '$route',
    'camAPI',
