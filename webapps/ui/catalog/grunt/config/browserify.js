...
module.exports = function(config, browserifyConfig) {
  ...
  browserifyConfig.catalog_scripts = {
    options: {
      browserifyOptions: {
        standalone: 'CamundaCatalogUi',
        debug: true
      },
      watch: true,
      transform: [[
      ...
      postBundleCB: function(err, src, next) {
        ...
        if (buildMode !== 'prod' && livereloadPort) {
          config.grunt.log.writeln(
            'Enabling livereload for catalog on port: ' + livereloadPort
          );
    ...
    src: [
      './<%= pkg.gruntConfig.catalogSourceDir %>/scripts/camunda-catalog-ui.js'
    ],
    dest: '<%= pkg.gruntConfig.catalogBuildTarget %>/scripts/camunda-catalog-ui.js'
  };

  browserifyConfig.catalog_plugins = {
    options: {
      watch: true,
      transform: [
        [
      ...
      browserifyOptions: {
        standalone: 'CatalogPlugins',
        debug: true
      }
    },
    src: [
      './<%= pkg.gruntConfig.pluginSourceDir %>/catalog/plugins/catalogPlugins.js'
    ],
    dest: '<%= pkg.gruntConfig.pluginBuildTarget %>/catalog/app/plugin.js'
  };
};
